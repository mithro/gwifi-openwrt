...
- **wisp.welland.mithis.com** (the OpenWISP VM) **moves onto VLAN 4**
  (10.1.5.2 → 10.1.4.2, single NIC) and owns DHCP + TFTP + HTTP for the
  pucks; **DNS stays on ten64** (D7), and wisp's DHCP answers point router
  and DNS at 10.1.4.1. Other VLANs reach OpenWISP via normal ten64 routing.
...
| D7 | VLAN-4 DNS | ten64 serves DNS/resolver for VLAN 4 (gdoc2netcfg-generated host-records); wisp serves **DHCP+TFTP only**, DHCP options point router+DNS at 10.1.4.1 | User decision after implementation finding: ten64's `auth-zone=welland.mithis.com` makes nested zone-forwards to wisp impossible (authoritative NXDOMAIN pre-empts `server=/wifi…/`). Generated host-records fit the existing "welland DNS is gdoc2netcfg data" architecture and give PTRs for free. |
...
- **ten64 dnsmasq@internal** (revised 2026-07-11 during implementation —
  D7): ten64 **is the DNS server and resolver for VLAN 4** but serves **no
  DHCP/boot** there:
  - `network-04-wifi.conf` follows the per-VLAN file pattern (interface +
    listen-address + `domain=`/`auth-zone=wifi.welland.mithis.com`) but has
    **no dhcp-range** and an explicit `no-dhcp-interface=br-wifi` with a
    comment naming this design — DHCP/TFTP-boot answers must only ever come
    from wisp.
  - Puck names/PTRs are **`host-record` lines generated by gdoc2netcfg**
    (identity is deterministic from the sheet) into `internal/generated/`.
    A zone-forward to wisp was tried and **cannot work**: the instance is
    authoritative for the parent `welland.mithis.com` (`auth-zone`), so
    nested subzone `server=/…/` forwards are pre-empted by authoritative
    NXDOMAIN (verified empirically).
...
- **Guest networking:** match the guest's existing network manager (verify at
  ...
  hang on HTTPS). Resolver: `10.1.4.1` — ten64's internal dnsmasq serves DNS
  on br-wifi (D7), on-link, no dependency on wisp's own services. Guest
  config staged **before** the libvirt retarget, keyed to the new MAC.
...
- **Ordering:** ten64 VLAN plumbing (5.1) must exist before the retarget;
  ...
  the new address.
- **dnsmasq (new package install, single instance) — DHCP+TFTP only (D7):**
  - Binds only the VLAN-4 interface (`bind-dynamic` + `interface=` scoping);
    **`port=0` — no DNS at all** (ten64 is the VLAN's DNS server; this also
    removes any systemd-resolved interaction).
  ...
  - `dhcp-option=option:router,10.1.4.1` and
    `dhcp-option=option:dns-server,10.1.4.1` (ten64 for both, per D7).
...
- **Emit `out/internal/gwifi-pucks-dns.conf`** (D7): one
  `host-record=puck<NN>.wifi.welland.mithis.com,10.1.4.<100+NN>` line per
  puck, deployed into ten64's `internal/generated/` by the existing copy
  procedure — this is how puck names (and PTRs) resolve site-wide.
...
- **Deployment:** the documented ten64 update procedure gains one step:
  ...
  The DNS fragment flows with the normal generated-config copy.
...
- Pucks: `puck<NN>.wifi.welland.mithis.com` → fixed IP, served **by ten64**
  as gdoc2netcfg-generated `host-record` lines (A + PTR) under its existing
  `welland.mithis.com` authority — same pipeline as every other welland
  device, derived from the same pucks tab as `pucks.json`.
- Pucks (and wisp) use ten64 (`10.1.4.1`) as their resolver; wisp's dnsmasq
  serves no DNS (`port=0`).
...
| Failure | Behaviour |
|---|---|
| wisp down / dnsmasq dead | Puck DHCP times out (bounded resends) → eMMC fallback. Production pucks unaffected beyond a boot delay. Puck DNS unaffected (ten64 serves it — D7). |
