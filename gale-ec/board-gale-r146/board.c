...
/* SPIKE-A forward-port: tcpm.h moved to driver/tcpm/tcpm.h on the include path. */
#include "tcpm/tcpm.h"
...
static struct usart_config const ap_usart =
	/*
	 * SPIKE-A forward-port: USART_CONFIG() gained a FLAGS argument between
	 * BAUD and the RX/TX queues. Gale needs no special USART flags -> 0.
	 */
...
const void *const usb_strings[] = {
	...
	/* SPIKE-A forward-port: descriptor for the USB-SPI bridge interface. */
...
/*
 * SPI devices.
 * SPIKE-A forward-port: spi_device_t gained a 4th field (USB-SPI flags); mark
 * this device USB_SPI_ENABLED so the USB-SPI bridge driver exposes it (the old
 * USB_SPI_CONFIG() instance macro was removed in favour of this flag plus a
 * usb_spi_enable() call in board_init()).
 */
const struct spi_device_t spi_devices[] = {
	{ CONFIG_SPI_FLASH_PORT, 0, GPIO_SPI_FLASH_NSS, USB_SPI_ENABLED },
};
const unsigned int spi_devices_used = ARRAY_SIZE(spi_devices);

/*
 * SPIKE-A forward-port: usb_spi_board_enable()/_disable() no longer take a
 * struct usb_spi_config* (the driver tracks state internally now).
 */
void usb_spi_board_enable(void)
{
	...
	if (!gpio_get_level(GPIO_SYS_PWR_EN)) {
		gpio_set_level(GPIO_SYS_PWR_EN, 1);
		gpio_set_level(GPIO_VDD_3P3_EN, 1);
		crec_usleep(25); /* SPIKE-A: usleep() -> crec_usleep() */
	} else {
	...
	/* SPIKE-A: spi_enable() now takes a spi_device_t*, not a port index. */
...
/*
 * SPIKE-A forward-port: the USB_SPI_CONFIG() instance macro was removed. The
 * USB-SPI bridge is now configured by CONFIG_USB_SPI + the USB_SPI_ENABLED flag
 * on the spi_devices[] entry, and started via usb_spi_enable(1) in board_init().
 */
...
const struct i2c_port_t i2c_ports[] = {
	/*
	 * SPIKE-A forward-port: i2c_port_t fields are now accessed by name
	 * (.name/.port/.kbps/.scl/.sda); positional init triggered a
	 * char*->int conversion error after a struct-layout change.
	 */
	{
		.name = "slave",
		.port = I2C_PORT_SLAVE,
		.kbps = 100,
		.scl = GPIO_SLAVE_I2C_SCL,
		.sda = GPIO_SLAVE_I2C_SDA,
	},
};
 ...
 */
/*
 * SPIKE-A forward-port: the usb_mux driver API changed. Callbacks now take
 * (const struct usb_mux *me) instead of an int port_addr, set() gained a
 * bool *ack_required out-param, usb_muxes[] is now an array of
 * struct usb_mux_chain (each pointing at a struct usb_mux), and the per-mux
 * .port_addr field was removed. This GPIO mux ignores `me`.
 */
static int board_ss_mux_set(const struct usb_mux *me, mux_state_t mux_state,
			    bool *ack_required)
{
	/* This mux completes synchronously; no host-command ACK needed. */
	*ack_required = false;

	/* SPIKE-A: MUX_* flags renamed to USB_PD_MUX_* (in ec_commands.h). */
	gpio_set_level(GPIO_USB_CC_POLARITY,
		       (mux_state & USB_PD_MUX_POLARITY_INVERTED) ? 0 : 1);
	gpio_set_level(GPIO_USB_SS_MUX_EN_L,
		       (mux_state & USB_PD_MUX_USB_ENABLED) ? 0 : 1);
	return EC_SUCCESS;
}

static int board_ss_mux_get(const struct usb_mux *me, mux_state_t *mux_state)
{
	/* SPIKE-A: MUX_USB_ENABLED renamed to USB_PD_MUX_USB_ENABLED. */
	*mux_state = gpio_get_level(GPIO_USB_SS_MUX_EN_L) ? 0
							  : USB_PD_MUX_USB_ENABLED;
	return EC_SUCCESS;
}

static int board_ss_mux_init(const struct usb_mux *me)
{
...
static const struct usb_mux board_ss_mux = {
...
/*
 * SPIKE-A forward-port: pd_send_host_event() is now provided by include/usb_pd.h
 * as a static-inline no-op whenever CONFIG_USB_PD_TCPM_STUB is set (gale's case),
 * so the board-local definition has been removed to avoid a redefinition.
 */
...
static void board_init(void)
{
	...
	/*
	 * SPIKE-A forward-port: with the USB_SPI_CONFIG() instance macro gone,
	 * the USB-SPI bridge endpoint must be explicitly enabled here (matches
	 * servo_micro). The spi_devices[] entry carries the USB_SPI_ENABLED flag.
	 */
	usb_spi_enable(1);
	 ...
	 *
	 * SPIKE-A forward-port: the usb_stream_state struct dropped the
	 * `rx_disabled` field (the USB-stream RX gate was removed upstream);
	 * struct usb_stream_state now exposes only `flags` (TX_FLUSH). There is
	 * no longer a driver knob to disable the AP-console RX path, so the
	 * locked branch is a no-op here. TODO(full-port): reinstate an
	 * RX-inhibit equivalent (e.g. gate at the USART or queue layer) to
	 * preserve the "locked AP console is read-only" behavior.
	 */
	if (system_is_locked())
		; /* no RX-disable knob on this base; see note above */
	else
...
static int command_polarity(int argc, const char **argv)
{
	if (!system_is_locked() && argc > 1) {
		gpio_set_level(GPIO_USB_CC_POLARITY,
...
static int command_dev(int argc, const char **argv)
{
	int v;
...
static int command_rec(int argc, const char **argv)
{
	int v;
...
static int command_gale(int argc, const char **argv)
{
	int i;
	...
	if (argc < 2) {
		...
		for (i = 0; i < ARRAY_SIZE(gale_subcommands); i++)
			/* SPIKE-A: console handlers now take const char **argv. */
			gale_subcommands[i].handler(1,
				(const char **)&gale_subcommands[i].name);
		return EC_SUCCESS;
	}
...
/*
 * SPIKE-A forward-port: DECLARE_CONSOLE_COMMAND dropped its trailing flags
 * argument (now NAME, ROUTINE, ARGDESC, HELP). Removed the trailing NULL.
 */
...
{
	/*
	 * SPIKE-A forward-port: tcpm_get_cc() now takes
	 * enum tcpc_cc_voltage_status * (was int *).
	 */
	 ...
	 *
	 * SPIKE-A forward-port: TYPEC_CC_VOLT_SNK_* were renamed to
	 * TYPEC_CC_VOLT_RP_*; the original "is Rp in [DEF..3_0]" range test is
	 * exactly the new cc_is_rp() helper from usb_pd_tcpm.h.
	 */
	if (cc_is_rp(cc1))
		return;
	if (cc_is_rp(cc2))
	 ...
	 *
	 * SPIKE-A forward-port: pd_set_dual_role() gained a leading port arg.
	 */
	pd_set_dual_role(0, system_is_locked() ? PD_DRP_TOGGLE_OFF
					       : PD_DRP_FORCE_SINK);
}
